...
def base64url_encode(input_bytes: bytes) -> bytes:
    ...
    return''.join(BASE64URL_TABLE[int(group, 2)] for group in groups).encode('ascii')


def base64url_decode(encoded: bytes) -> bytes:
    input_bits = ''.join(f'{BASE64URL_TABLE.index(chr(byte)):0{GROUPS_LEN_BITS}b}' for byte in encoded)
